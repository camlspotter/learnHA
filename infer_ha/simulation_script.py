...
def generate_simulation_script(out : TextIOWrapper,
    ...
    if (len(input_variables) > 0):
        ...
        for varName in input_variables:
            ...
            out.write(f"ds = ds.addElement({timeseries_varName}, '{input_port_Name}');\n%%\n")

        out.write("set_param(mdl, 'LoadExternalInput', 'on');\n")
        out.write("set_param(mdl, 'ExternalInput', 'ds'); % Set the initial input values in the model.\n")
    ...
    output_str += ", ".join(# Writing first the input variables, which is all followed by the output ports
                            [ f"y( : , {i+1})"
                              for i
                              in range(len(output_variables), len(input_variables) + len(output_variables))] +

                            # Writing then the output variables, which are the starting output ports
                            [ f"y( : , {i+1})"
                              for i
                              in range(0, len(output_variables)) ])
