...
# Table of C++ option => Python option
# segment-relative-error -> segmentation-error-tol
# segment-relative-fine-error -> segmentation-fine-error-tol (no longer used) 
# lmm-step-size -> lmm-step-size
# precision-equivalence -> counter-example-threshold (but with a different distance now)
# correlation-threshold -> threshold-correlation
# distance-threshold -> threshold-distance
# max-generate-trace-size -> not used
# max-traces 3 --> no longer used in python
# max-stoptime 20 --> inference loop timeout. not used
