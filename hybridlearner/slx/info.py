...
def get_IOports(fn: str) -> tuple[list[str], list[str]]:
    ...
    engine.eval0(f"""
        ...
        inports = getfullname(Simulink.findBlocksOfType(mdl, 'Inport', opts));
        outports = getfullname(Simulink.findBlocksOfType(mdl, 'Outport', opts));
    """)
...
def get_all_params(fn: str) -> Any:
    ...
    engine.eval0(f"""
        modelPath = '{fn}'; 
        mdl = load_system(modelPath); 
        res = get_param(mdl, 'ObjectParameters');
    """)
    return engine.getvar('res')
