...
def dbscan_cluster(
    ...
    res = create_simple_per_segmented_positions(
        segmented_traj
    )  # for ODE inference we use segment excluding boundary points
    selected_array_coeff = convert_clfs_to_coeffArray_selected_data(
        clfs, size_of_input_variables
    )
    ...
    db = skc.DBSCAN(eps=dbscan_eps_dist, min_samples=dbscan_min_samples).fit(
        selected_array_coeff
    )
