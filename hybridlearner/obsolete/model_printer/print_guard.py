def print_guard(f_out, coeffs, gExp):
    ...
    for i in range(0, len(coeffs)):  # for each term of the polynomial
        coef_val = coeffs[i]
        # divide only when first coefficient is >1 to normalize and not spoil/raise values
        if (coef1 != 0) and (coef1 > 1 or coef1 < 1):
            # Dividing all coefficient by the 1st coefficient, since right hand side of the equation is 0
            coef_val = coef_val / coef1
            # coef_val = coef_val
