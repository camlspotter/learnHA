...
def report(
    ...
    passed: Optional[list[tuple[Trajectory, Trajectory]]],
    ...
    with utils_io.open_for_write(log_filename) as log:
        ...
        if len(trs) != 0:
            ...
            plot_timeseries_multi(counter_example_file, 'Counter examples', header, trs)
        ...
        if passed is None:
            ...
            trs = [
                (
                    f"Trajectory {i}",  # title
                    (
                        ot[0],  # time
                        np.hstack(
                            (ot[1], lt[1][:, -len(output_variables) :])
                        ),  # values
                    ),
                )
                for (i, (ot, lt)) in enumerate(passed_10)
            ]
            ...
            if len(trs) != 0:
                ...
                plot_timeseries_multi(
                    passed_example_file, 'Passed examples', header, trs
                )
