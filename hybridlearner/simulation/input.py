...
def linear_signal(time_horizon: float, cps: list[float]) -> Signal:
    """
...
def fixed_step_signal(time_horizon: float, cps: list[float]) -> Signal:
    """
...
def generate_input_value_ts(
    ...
    return {
        v: build_signal(
            rng, time_horizon, invariant[v], number_of_cps[v], signal_types[v]
        )
        for v in input_variables
    }


def generate_output_values(
    rng: random.Random, invariant: Invariant, output_variables: list[str]
) -> dict[str, float]:
...
def generate_simulation_input(
    ...
    input_value_ts = generate_input_value_ts(
        rng, time_horizon, invariant, number_of_cps, signal_types, input_variables
    )
